...
def cargar_datos(f_datos, f_restricciones):
    ...
    for i in range(len(matrix_const)):
        for j in range(i,len(matrix_const[0])):
...
def calcular_infeasibility(datos, valores_asignados, restricciones):
    ...
    for i in range(len(restricciones)):
        if restricciones[i][2] == '-1' and restricciones[i][0] == restricciones[i][1]:
            infeasability = infeasability + 1
        elif restricciones[i][2] == '1' and restricciones[i][0] != restricciones[i][1]:
            infeasability = infeasability + 1
...
def cruce(num_esperado_cruces, poblacion, n_genes,d,datos,restricciones,valor_lambda, num_evaluaciones):
    ...
    if elitista == True:
        while contador < num_esperado_cruces:
            ...
            recalcular_centroides(datos, p_intermedia[individuo].valores_asignados, p_intermedia[individuo].centroides, d)

            p_intermedia[individuo].vf_objetivo = f_objetivo(datos, p_intermedia[individuo].valores_asignados, restricciones, n, d, p_intermedia[individuo].centroides, valor_lambda)
            num_evaluaciones = num_evaluaciones + 1
            ...
            recalcular_centroides(datos, p_intermedia[individuo + 1].valores_asignados, p_intermedia[individuo + 1].centroides, d)

            p_intermedia[individuo + 1].vf_objetivo = f_objetivo(datos, p_intermedia[individuo + 1].valores_asignados, restricciones, n, d, p_intermedia[individuo + 1].centroides, valor_lambda)
            num_evaluaciones = num_evaluaciones + 1
    ...
    return p_intermedia, num_evaluaciones


def mutacion(num_esperado_mutacion, poblacion ,n_genes,prob_muta, long_poblacion, datos, restricciones, d, valor_lambda, num_evaluaciones):
    ...
    if elitista == True:
        if prob_muta > 0:
            ...
            while contador < num_esperado_mutacion:
                ...
                for i in range(k):
                    if cluster_vacio(poblacion[i].valores_asignados,i+1) == 0:
                        #reparar = True
                        reparacion(poblacion[i],i+1,n_genes)
                    break
        ...
        for i in range(k):
            if cluster_vacio(poblacion[0].valores_asignados,i+1) == 0:
                #reparar = True
                reparacion(poblacion[0],i+1,n_genes)
                break
            
            if cluster_vacio(poblacion[1].valores_asignados,i+1) == 0:
                #reparar = True
                reparacion(poblacion[1],i+1,n_genes)
                break


                     
        recalcular_centroides(datos, poblacion[0].valores_asignados, poblacion[0].centroides, d)
        poblacion[0].vf_objetivo = f_objetivo(datos, poblacion[0].valores_asignados, restricciones, n, d, poblacion[0].centroides, valor_lambda)
        num_evaluaciones = num_evaluaciones + 1


        recalcular_centroides(datos, poblacion[1].valores_asignados, poblacion[1].centroides, d)
        poblacion[1].vf_objetivo = f_objetivo(datos, poblacion[1].valores_asignados, restricciones, n, d, poblacion[1].centroides, valor_lambda)

        num_evaluaciones = num_evaluaciones + 1

    return poblacion, num_evaluaciones
...
def reemplazar_poblacion(poblacion_actual, poblacion_hijos, mejor_cromosoma, peor_cromosoma, pos_peor, long_poblacion, datos, restricciones, n, d, valor_lambda, num_evaluaciones):
    ...
    if elitista == True:
        ...
        if poblacion_hijos[el_mejor].vf_objetivo < peor_cromosoma.vf_objetivo:
            poblacion_actual[pos_peor].valores_asignados = np.copy(poblacion_hijos[el_mejor].valores_asignados)
            poblacion_actual[pos_peor].vf_objetivo = np.copy(poblacion_hijos[el_mejor].vf_objetivo)
            poblacion_actual[pos_peor].centroides = np.copy(poblacion_hijos[el_mejor].centroides)

            #Si el segundo mejor es mejor que el segundo peor
            if poblacion_hijos[segundo_mejor].vf_objetivo < peor_cromosoma2.vf_objetivo:
                poblacion_actual[pos_peor2].valores_asignados = np.copy(poblacion_hijos[segundo_mejor].valores_asignados)
                poblacion_actual[pos_peor2].vf_objetivo = np.copy(poblacion_hijos[segundo_mejor].vf_objetivo)
                poblacion_actual[pos_peor2].centroides = np.copy(poblacion_hijos[segundo_mejor].centroides)   
...
def BL_suave(cromosoma, k, num_fallos_max, n_genes, d, datos, restricciones, valor_lambda, num_evaluaciones):
    ...
    return cromosoma , num_evaluaciones
...
def AG(datos, restricciones, n, d, long_poblacion, prob_cruce, prob_muta):
    ...
    while(num_evaluaciones <= NUM_MAX_EVALUACIONES):
        ...
        p_cruzada, num_evaluaciones = cruce(num_esperado_cruces, nueva_poblacion,n,d,datos,restricciones,valor_lambda, num_evaluaciones)

        p_hijos, num_evaluaciones = mutacion(num_esperado_mutacion, p_cruzada,n,prob_muta,long_poblacion,datos,restricciones,d,valor_lambda, num_evaluaciones)
...
def AGE(datos, restricciones, n, d, long_poblacion, prob_cruce, prob_muta):
    num_evaluaciones = 0
    num_esperado_cruces = 1
    ...
    for f2 in listdir("restricciones"):
        ...
        if txt_datos[0] == txt_restricciones[0]:
            ...
            for c in range(5):
                ...
                v_sol_AGE_UN, desv_AGE_UN, valor_inf_AGE_UN, valor_fobjetivo_AGE_UN = AGE(datos, restricciones, n,d,long_poblacion,prob_cruce,prob_muta)
                ...
                v_sol_AGE_SF, desv_AGE_SF, valor_inf_AGE_SF, valor_fobjetivo_AGE_SF = AGE(datos, restricciones, n,d,long_poblacion,prob_cruce,prob_muta)
...
prob_cruce = 1
prob_muta = 0.01
v_sol, desv, valor_inf, valor_fobjetivo = AGE(datos, restricciones, n,d,long_poblacion,prob_cruce,prob_muta)
