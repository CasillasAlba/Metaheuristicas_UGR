...
def recalcular_centroides(datos, vector_asignados, centroides, d):
    ...
    for i in range(k):
        ...
        for j in range(d):
            if centroides[i - 1][j] != nuevo_centroide[j]:
                ha_cambiado = 1 #HA CAMBIADO 
            


        centroides[i -1] = nuevo_centroide
...
def calcular_infeasibility(datos, valores_asignados, restricciones):
    ...
    print ("TAM V ASIGNADOS")
    print(len(valores_asignados))

    for i in range(len(valores_asignados)):

        if valores_asignados[i] != 0:

            for j in range(len(valores_asignados)):

                print("ITERACIONES ", i , " " , j)

                print("MATRIX RESTRIC")
                print(restricciones[i][j]) 
                print("VALOR DE VALORES ASIGNADOS DE i Y j")
                print(valores_asignados[i])
                print(valores_asignados[j])

                print("VALOR INFEASIBILITY " , infeasability)
                
                if restricciones[i][j] == '-1' and valores_asignados[i] == valores_asignados[j]:
                    print("HE ENTRADO EN EL IF 1")
                    infeasability = infeasability +1
                
                if restricciones[i][j] == '1' and valores_asignados[i] != valores_asignados[j]:
                    print("HE ENTRADO EN EL IF 2")
                    infeasability = infeasability +1

                print("VALOR INFEASIBILITY DESPUES " , infeasability)
...
def calcular_lambda(datos, restricciones, n):
    ...
    la_distancia = distancima_maxima + 100
...
def Greedy(datos, restricciones, centroides, n, d):
    ...
    while (hay_cambio != 0) and (contador < MAX_ITER):
        vector_asignados = np.zeros(n)
        contador += 1
        ...
        hay_cambio = recalcular_centroides(datos, vector_asignados, centroides, d)

    
    desviacion_general = desviacion(datos, centroides, d, vector_asignados)
    valor_inf = calcular_infeasibility(datos, vector_asignados, restricciones)/n
...
def LocalSearch(datos, restricciones, n, d):
    ...
    v_centroides = np.zeros((k,d))

    centroides_cambiados = recalcular_centroides(datos, v_solucion, v_centroides, d)
    ...
    while(hay_cambio == 1) and (contador < MAX_ITER):
        ...
        #de que me sirve este for de fuera si dentro de recalcular centoride ya hay un for que recorre las n posciones????
        for v in range(len(vecinos)):
            centroides_cambiados = recalcular_centroides(datos, vecinos[v], v_centroides, d)

        for v in range(len(vecinos)):
            sol1 = f_objetivo(datos, vecinos[v], restricciones, n, d, centroides, valor_lambda)
            print("SOL UNO ", sol1)
