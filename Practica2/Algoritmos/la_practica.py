...
def torneo_binario(poblacion, long_poblacion):
    ...
    cromosoma1 = poblacion[aleatorio1]
    cromosoma2 = poblacion[aleatorio2]

    if cromosoma1.vf_objetivo < cromosoma2.vf_objetivo:
        return cromosoma1
    else:
        return cromosoma2
...
def seleccionar_poblacion(long_poblacion,poblacion):
    ...
    for i in range(tope):
        nueva_poblacion.append(torneo_binario(poblacion, long_poblacion))
...
def operador_uniforme(padre1, padre2, n_genes,d):
    ...
    for i in range(int(n_genes/2)):
        aleatorio = random.randint(0,(n_genes-1))
        hijo.valores_asignados[int(aleatorio)] = np.copy(padre1.valores_asignados[int(aleatorio)])
...
def segmento_fijo(padre1, padre2, n_genes,d):
    ...
    for i in range(int(restantes/2)):
        aleatorio = random.randint(posicion,(posicion + (n_genes - tam_segmento)))%n_genes
        hijo.valores_asignados[int(aleatorio)] = np.copy(padre1.valores_asignados[int(aleatorio)])
...
def cruce(num_esperado_cruces, poblacion, n_genes,d):
    ...
    while contador < num_esperado_cruces:
        ...
        aux = np.bincount(np.array(hijo.valores_asignados, dtype=int))

        for i in range(len(aux)):
            if int(aux[i]) == 0:
                reparacion(hijo,i+1,n_genes)

        aux_otro = np.bincount(np.array(otro_hijo.valores_asignados, dtype=int))
