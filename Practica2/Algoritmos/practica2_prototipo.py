...
def reemplazar_poblacion(poblacion_actual, poblacion_hijos, mejor_cromosoma, peor_cromosoma, pos_peor, long_poblacion, datos, restricciones, n, d, valor_lambda, num_evaluaciones):
    ...
    if elitista == True:
        ...
        peor_cromosoma2, pos_peor2 = segundo_peor(poblacion_hijos, peor_cromosoma)
        ...
        if poblacion_hijos[el_mejor].vf_objetivo < peor_cromosoma.vf_objetivo:
            ...
            poblacion_actual[pos_peor].vf_objetivo = np.copy(poblacion_hijos[el_mejor].valores_asignados)
            ...
            if poblacion_hijos[segundo_mejor].vf_objetivo < peor_cromosoma2.vf_objetivo:
                ...
                poblacion_actual[pos_peor2].vf_objetivo = np.copy(poblacion_hijos[segundo_mejor].valores_asignados)
    ...
    while(num_evaluaciones <= NUM_MAX_EVALUACIONES):
        ...
        if (num_generaciones%generaciones) == 0:
            if tipo_mej == True:
                #ordenar poblacion
                #np.sort(p_hijos)
                # for i in range(np.int(tasa*len(p_hijos))):
                    #calcular busqueda local
                    #p_hijos[i] = BL_suave(p_hijos[i], k, num_fallos_max, n, d, datos, restricciones, valor_lambda, num_evaluaciones)
                print("aqui es solo para los 10 mejores")
            else:
